...
public class AnimationSpritePerso extends AnimationTimer {
    ...
    public AnimationSpritePerso(Grille grille, StackPane SpritesPerso) {
        ...
        this.decalage = GameLoop.TUILE_TAILLE - GameLoop.TUILE_TAILLE/3;
        this.running = false;
    }
    ...
    public void handle(long now) {
        if (now - lastUpdate >= latence) {
            ...
            immobile();
            decalage = GameLoop.TUILE_TAILLE - GameLoop.TUILE_TAILLE/3;
            stop();
        }
    ...
    private void animation() {
        ...
        switch (grille.getPerso().getDirPrecedente()) {
            case haut:
                spritesPerso.setTranslateX(grille.getPerso().getX() * (GameLoop.TUILE_TAILLE));
                spritesPerso.setTranslateY(grille.getPerso().getY() * (GameLoop.TUILE_TAILLE) + decalage);
            ...
            case bas:
                ...
                break;
            case gauche:
                spritesPerso.setTranslateX(grille.getPerso().getX() * (GameLoop.TUILE_TAILLE) + decalage);
                spritesPerso.setTranslateY(grille.getPerso().getY() * (GameLoop.TUILE_TAILLE));
        ...
        decalage-=GameLoop.TUILE_TAILLE/3;
    }
